...
# def setup_openai():
#     """Configure OpenAI API with API key and return model name."""
#     if not openai.api_key:
#         raise ValueError("OpenAI API key is not set")
#
#     OPENAI_API_KEY = openai.api_key
#     console.print("[bold green]✓[/bold green] OpenAI API configured successfully")
    #return OPENAI_MODEL  # 返回模型名称，用于后续调用
def setup_openai():
    console.print("[bold green]✓[/bold green] OpenAI API configured successfully")
    return "gpt-4o"
...
def main():
    ...
    # Setup Gemini
    try:
        OPENAI_MODEL = setup_openai() ###    setup_gemini()
    except Exception as e:
    ...
    # chat = OPENAI_MODEL.start_chat(history=[]) ###
    ...
    while True:
        ...
        processed_response = process_llm_response(reply)
